...
export const INVALID_PRESENCE_COUNT_COMBO = {
...
function _resolvePresence(value) {
  ...
  if (!isValidPresence(p)) {
    throw new Error(
      `ConceptSystemMember.presence has invalid value ${JSON.stringify(value)}; ` +
...
function _resolveCount(value) {
  ...
  if (!isValidCount(c)) {
    throw new Error(
      `ConceptSystemMember.count has invalid value ${JSON.stringify(value)}; ` +
