// LegacyPartitiveHyperedge — a one-to-many partitive decomposition.
//
...
export class LegacyPartitiveHyperedge extends GlossaristModel {
  constructor(data = {}) {
    super();
  ...
  identity() {
    return LegacyPartitiveHyperedge.identityOf(this);
  }

  static fromJSON(data) {
    return new LegacyPartitiveHyperedge(data);
  }
}
...
function ensureConceptRef(value, fieldName) {
  ...
  if (!ref.source && !ref.id) {
    throw new Error(
      `LegacyPartitiveHyperedge#${fieldName} must be a non-empty ConceptRef ` +
...
function assertNonEmptyParts(parts) {
  if (!parts || parts.length === 0) {
    throw new Error('LegacyPartitiveHyperedge requires at least one part');
  }
}
...
function assertNoSelfLoop(comprehensive, parts) {
  ...
  for (const p of parts) {
    ...
    if (pKey === cKey) {
      throw new Error('LegacyPartitiveHyperedge#parts cannot include the comprehensive');
    }
  }
}
...
//
// Cross-repo symmetry: glossarist-ruby's `LegacyPartitiveHyperedge#content=`
